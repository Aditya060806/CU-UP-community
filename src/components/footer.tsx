  ...
  const contactItems = [
    {
      ...
      icon: Github,
      label: "github.com/tarunagnihotri534",
      clickable: true,
    },
    {
      icon: Mail,
      label: "tarunagnihotri534@gmail.com",
      clickable: true,
  ...
  const SocialSection = ({
    ...
    <div className="space-y-6">
      <h3 className="text-xl font-bold text-slate-900 dark:text-slate-100 relative">
        {title}
        <div className="absolute -bottom-2 left-0 h-0.5 w-12 bg-linear-to-r from-red-600 to-amber-500" />
      </h3>
        ...
        {items.map((link) => (
          <li key={link.href}>
            <a
              ...
              className="group flex items-center gap-2 text-slate-600 dark:text-slate-300 hover:text-red-700 dark:hover:text-red-400 transition-all duration-300"
            >
              <div className="p-1 rounded-md bg-slate-100 dark:bg-slate-800 group-hover:bg-red-100 dark:group-hover:bg-red-900/30 transition-colors">
          ...
          <li key={link.href}>
            {isNav ? (
              <button
                ...
                className="group flex items-center gap-2 text-slate-600 dark:text-slate-300 hover:text-red-700 dark:hover:text-red-400 transition-all duration-300 cursor-pointer text-left"
              >
                <div className="w-1 h-1 rounded-full bg-slate-400 group-hover:bg-red-600 transition-colors" />
                <span className="text-sm font-medium group-hover:translate-x-1 transition-transform duration-300">
                  {link.label}
                </span>
              </button>
      ...
      <div className="relative container mx-auto px-4 py-20 max-w-7xl">
        ...
        >
          ...
          >
            <Link href="/" className="inline-block group">
              <span className="text-4xl md:text-5xl font-black bg-clip-text text-transparent bg-linear-to-r from-red-700 via-red-600 to-amber-500 dark:from-red-400 dark:via-red-400 dark:to-amber-400 hover:scale-105 transition-transform">
                {siteConfig.name}
              </span>
            </Link>
              ...
              {contactItems.map((item) => (
                <div key={item.label}>
                  {item.clickable && item.href ? (
                    ...
                    >
                      <div className="p-2 rounded-lg bg-slate-100 dark:bg-slate-800 group-hover:bg-red-100 dark:group-hover:bg-red-900/30 transition-colors">
                        <item.icon className="h-4 w-4 group-hover:text-red-700 dark:group-hover:text-red-400 transition-colors" />
                      </div>
        ...
        >
          <p className="text-sm text-slate-500 dark:text-slate-400 text-center md:text-left">
            ...
            reserved. &mdash; Chandigarh University, Uttar Pradesh.
          </p>
