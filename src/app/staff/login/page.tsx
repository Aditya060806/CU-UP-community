...
export default function StaffLogin() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (data.role !== "staff") { setError("Access denied. Staff accounts only."); return; }
    ...
    <div className="min-h-screen bg-gradient-to-br from-zinc-950 via-red-950 to-zinc-900 flex items-center justify-center p-4">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-zinc-400 text-sm mt-1">CU-UP — Authorized Staff Only</p>
        </div>
        ...
        <Card className="border-zinc-800 bg-zinc-900/90 backdrop-blur-sm shadow-2xl">
          <CardHeader className="pb-2">
            ...
            <CardDescription className="text-zinc-400">
              Default: <code className="text-amber-400 text-xs">staff@cuup.in</code> / <code className="text-amber-400 text-xs">Staff@2024</code>
            </CardDescription>
          </CardHeader>
          <CardContent>
            {error && (
            ...
            <p className="text-center text-xs text-zinc-500 mt-4">
              Student?{" "}
              <Link href="/student/login" className="text-amber-400 hover:underline">Student Portal →</Link>
            </p>
          </CardContent>
        </Card>
      </motion.div>
